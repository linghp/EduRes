...
public class DownloadItem extends RelativeLayout {
   ...
   public static interface DownloadSelectListener {
    ...
    private void initialize() {
        if (CHECKMARK_AREA == -1) {
            CHECKMARK_AREA = getResources().getDimensionPixelSize(R.dimen.checkmark_toucharea);
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch(event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                if (mIsInDownEvent && event.getX() < CHECKMARK_AREA&&mCheckBox.getVisibility()==VISIBLE) {
                    toggleCheckMark();  //不在编辑状态时，点击该区域也会弹出删除按钮
                    handled = true;
             }
                mIsInDownEvent = false;
                break;
